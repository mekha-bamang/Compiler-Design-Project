...
s = word;
switch(p->type) {
case typeCon: sprintf (word, "num(%d)", p->con.value); break;
case typeString: sprintf (word, "%s", p->str.val); break;
case typeId: sprintf (word, "id(%s)", p->id.i); break;
case typeFlt: sprintf (word, "flt(%f)", p->flt.value); break;
case typeBool: sprintf (word, "bool(%s)", p->bool.val); break;
case typeOpr:
switch(p->opr.oper){
...
*cm = c + w / 2;
/* node is leaf */
if (p->type == typeCon || p->type == typeId || p->type == typeBool || p->type == typeString || p->opr.nops == 0) {
